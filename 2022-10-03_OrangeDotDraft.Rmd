...
In the larger Charlottesville community, 9,413 families (14 percent) do not make enough money to afford the essentials of life—food, shelter, clothing and utilities—and the added costs associated with
working—childcare and transportation. This number represents the families making less than $35,000 in our region. 
...
__In the tables below, the cost of food, clothing, utilities, and transportation are taken from either national or region reports, so they are constant across all localities in the Charlottesville region. However, the cost of shelter and childcare vary widely by locality, and these expenses are also typically the largest expenses and the two that dictate how much a family needs to earn in order to survive without assistance.__
...
It's worth noting here that if families make just over >$35,000/year, it would still be nearly impossible to afford important expenses like childcare and transportation. These families would have almost no ability to save money or create a sense of financial security. To afford every expense in this table, save for your child's future education, a downpayment on a home, or any unexpected cost, a family would realistically likely need to be making >=$50k. However, we focus on families making <$35k/year because in every locality, that amount is at least greater than the survival expenses. 
...
leaflet() %>% 
  ...
  addPolygons(data = ACmap,
              ...
              popup = paste0("Median family income: ", "$", comma(ACmap$medianFamIncomeE), "<br>",
                             "Area: ", ACmap$tractname)) %>% 
  addLegend("bottomright", pal = pal, values = ACmap$medianFamIncomeE, 
            title = "Median family income", opacity = 0.7)
```

The Carr's Hill-McCormick Road (UVA) tract has one of the lowest median family incomes at `r paste0(comma(ACmap$medianFamIncomeE[ACmap$tract == "010904"]))`. This tract likely includes graduate students and other low-paid families employed by the University. By contrast, the Ivy and North Garden tracts have some of the highest median family incomes, and they likely include many University professors and high-paid administrators. 
...
Click on each tract to see its median income and a nickname for the tract based on which neighborhood the tract contains. 
...
leaflet() %>% 
  ...
  addPolygons(data = BCmap,
              ...
              popup = paste0("Median family income: ", "$", comma(BCmap$medianFamIncomeE), "<br>",
                             "Area: ", BCmap$tractname)) %>% 
  addLegend("bottomright", pal = pal, values = BCmap$medianFamIncomeE, 
            title = "Median family income", opacity = 0.7)
```

The New Canton tract has one of the lowest median family incomes at `r paste0(comma(BCmap$medianFamIncomeE[BCmap$tractnames == "New Canton"]))`. However, relative to the other localities, the difference between the lowest median family income and the highest median family income in Buckingham is small---the Mount Rush tract has the highest median family income with `r paste0(comma(BCmap$medianFamIncomeE[BCmap$tractnames == "Mount Rush"]))`. 
...
leaflet() %>% 
  ...
  addPolygons(data = CCmap,
              ...
              popup = paste0("Median family income: ", "$", comma(CCmap$medianFamIncomeE), "<br>",
                             "Area: ", CCmap$tractname)) %>% 
  addLegend("bottomright", pal = pal, values = CCmap$medianFamIncomeE, 
            title = "Median family income", opacity = 0.7)
```

The 10th & Page-Venable and the JPA-Fontaine tracts have the lowest median family incomes at `r comma(CCmap$medianFamIncomeE[CCmap$tractnames == "10th & Page-Venable"])` and `r comma(CCmap$medianFamIncomeE[CCmap$tractnames == "JPA-Fontaine"])` respectively. By contrast, the Barracks-Rugby tract has the highest median family income at `r comma(CCmap$medianFamIncomeE[CCmap$tractnames == "Barracks-Rugby"])`, making the disparity between the lowest and highest median family incomes well-over $100k. 
...
leaflet() %>% 
  ...
  addPolygons(data = FCmap,
              ...
              popup = paste0("Median family income: ", "$", comma(FCmap$medianFamIncomeE), "<br>",
                             "Area: ", FCmap$tractname)) %>% 
  addLegend("bottomright", pal = pal, values = FCmap$medianFamIncomeE, 
            title = "Median family income", opacity = 0.7)
```

The Columbia/Fork Union tract has the highest median family incomes at `r paste0(comma(FCmap$medianFamIncomeE[FCmap$tractnames == "Columbia/Fork Union"]))`. However, relative to the other localities, the difference between the lowest median family income and the highest median family income in Fluvanna is small. Additionally, the highest median family income in Fluvanna is much lower than the highest median family income in Albemarle or Charlottesville. 
...
leaflet() %>% 
  ...
  addPolygons(data = GCmap,
              ...
              popup = paste0("Median family income: ", "$", comma(GCmap$medianFamIncomeE), "<br>",
                             "Area: ", GCmap$tractname)) %>% 
  addLegend("bottomright", pal = pal, values = GCmap$medianFamIncomeE, 
            title = "Median family income", opacity = 0.7)
```

The Stanardsville tract has the lowest median family incomes at `r paste0(comma(GCmap$medianFamIncomeE[GCmap$tractnames == "Stanardsville"]))', and the Midway tract has the highest family median income at `r paste0(comma(GCmap$medianFamIncomeE[GCmap$tractnames == "Midway"]))'. 
...
leaflet() %>% 
  ...
  addPolygons(data = LCmap,
              ...
              popup = paste0("Median family income: ", "$", comma(LCmap$medianFamIncomeE), "<br>",
                             "Area: ", LCmap$tractname)) %>% 
  addLegend("bottomright", pal = pal, values = LCmap$medianFamIncomeE, 
            title = "Median family income", opacity = 0.7)
```

The Town of Louisa tract has the lowest median family incomes at `r paste0(comma(LCmap$medianFamIncomeE[LCmap$tractnames == "Town of Louisa"]))', and the Zion Crossroads tract has the highest family median income at `r paste0(comma(LCmap$medianFamIncomeE[LCmap$tractnames == "Zion Crossroads"]))'. 
...
leaflet() %>% 
  ...
  addPolygons(data = NCmap,
              ...
              popup = paste0("Median family income: ", "$", comma(NCmap$medianFamIncomeE), "<br>",
                             "Area: ", NCmap$tractname)) %>% 
  addLegend("bottomright", pal = pal, values = NCmap$medianFamIncomeE, 
            title = "Median family income", opacity = 0.7)
```

The Lovingston tract has the lowest median family incomes at `r paste0(comma(NCmap$medianFamIncomeE[NCmap$tractnames == "Lovingston"]))', and the Zion Crossroads tract has the highest family median income at `r paste0(comma(NCmap$medianFamIncomeE[NCmap$tractnames == "Wintergreen-Rockfish Valley"]))'. 
