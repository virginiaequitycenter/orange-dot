...
faminc_all <- read_csv("../data/faminc_county_2022.csv")
faminc_race <- read_csv("../data/faminc_race_county_2022.csv") 

faminc <- faminc_all %>% 
  rbind(faminc_race) %>% 
  filter(label != "Total Families") %>% 
  mutate(inc_group = case_when(str_detect(variable, "_002|_003|_004|_005|_006|_007") ~ "Earning below $35K",
...
faminc_data <- faminc %>% 
  group_by(GEOID, locality, year, group, inc_group, inc_rank) %>% 
...
faminc_data %>% 
  arrange(inc_rank) %>%
    ...
    text = case_when(percent >= 1 ~ paste0(round(percent, 0), "%\n(", prettyNum(estimate, big.mark=",", preserve.width="none"), ")"),
                     percent < 1 ~ "")) %>%
    ggplot(aes(x = group, y = percent, group = group, fill = inc_group, 
               label = text)) +
    geom_bar(stat = "identity", color = "white") + 
